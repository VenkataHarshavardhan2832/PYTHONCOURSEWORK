...
function one : max_element_in_array(arr) -> max_element:int

example : [1,2,3,4,5,10,-1,-2]
ans = 10

function two : min_element_in_arry(arr) -> min_element: int
Write an another function minmum_element which finds the minimum element in the array
example : [1,2,3,4,5,10,-1,-2]
ans = -2
'''
arr = [1,2,3,4,5,10,-1,-2]
def max_element_in_array(arr):
    max_element = 0
    for number in arr:
        if number > max_element:
            max_element = number
    return max_element
print(max_element_in_array(arr))